...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
    	...
    	print("[Epoch: {0}/{1} Loss: {2:2f} Time: {3:2f}]".format(epoch + 1, epochs, loss, time.time() - start_time))
...
def run():
	...
	# Load Data
	data_folder='./data_cityscapes/'
	image_paths = data_folder + 'leftImg8bit/'
	label_paths = data_folder + 'gtFine/'
	vgg_path = './data/vgg/'
	runs_dir = './runs_city'
	...
	with tf.Session() as sess:
		...
		### Testing ###
		image_test, gt_test = load_data(image_paths, label_paths, data_type='test')
		data_dir = './data_cityscapes/leftImg8bit/test/berlin/*.png'
		# save_inference_samples(runs_dir, data_dir, sess, img_shape, logits, keep_prob, input_image, label_colors)
		save_inference_samples(runs_dir, image_test, gt_test, sess, img_shape, logits, keep_prob, input_image, label_colors)
	...
	with tf.Session() as sess:
		...
		for epoch in range(epochs):
			...
			for image, label in get_batches_fn(batch_size):
				...
				batch_num += 1
				total_loss.append(loss)
				print("Batch {0} Loss {1} Avg.loss {2} Time {3}".format(batch_num, loss, np.mean(total_loss), time.time() - start_time))

			print("[Epoch: {0}/{1} Loss: {2:2f} Time: {3:2f}]".format(epoch + 1, epochs, np.mean(total_loss), time.time() - start_time))
